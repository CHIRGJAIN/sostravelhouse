...
export default function Blog() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="gradient-primary py-16"><div className="container mx-auto px-4 text-center"><h1 className="font-heading text-3xl font-bold text-primary-foreground">Travel Blog</h1><p className="mt-2 text-primary-foreground/80">Tips, guides, and inspiration for your next trip</p></div></div>
      <div className="container mx-auto px-4 py-12">
        {/* Featured */}
        {featured && (
          <Link to={`/blog/${featured.slug}`} className="group mb-12 grid gap-6 overflow-hidden rounded-2xl border border-border bg-card md:grid-cols-2">
            ...
            <div className="flex flex-col justify-center p-6">
              ...
              <div className="mt-4 flex items-center gap-3 text-xs text-muted-foreground">
                <span>{featured.author}</span><span>·</span><span>{featured.date}</span><span>·</span><Clock className="h-3 w-3" /><span>{featured.readTime}</span>
              </div>
            </div>
          </Link>
        )}
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {rest.map(post => (
            <Link key={post.id} to={`/blog/${post.slug}`} className="group overflow-hidden rounded-2xl border border-border bg-card transition-all hover:shadow-lg hover:-translate-y-1">
              ...
              <div className="p-5">
                <div className="flex flex-wrap gap-1.5 mb-2">{post.tags.map(t => <span key={t} className="rounded-full bg-muted px-2 py-0.5 text-[10px] font-medium text-muted-foreground">{t}</span>)}</div>
