...
const addOnOptions = [
  { k: "insurance", l: "Travel Insurance", price: 999, note: "per person" },
  { k: "pickup", l: "Airport Pickup", price: 1500, note: "one-way" },
  { k: "meals", l: "Meal Upgrade", price: 2000, note: "per person" },
];
...
export default function Booking() {
  ...
  const [bookingId] = useState(() => `ENG-${Date.now().toString(36).toUpperCase()}`);
  ...
  if (done) {
    return (
      <div className="min-h-screen bg-background">
        <PageHeader title="Booking Confirmed" subtitle="Your trip details are on the way." />
        <div className="container mx-auto px-4 py-12">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="mx-auto max-w-md card-surface p-8 text-center">
            ...
            <Button onClick={() => toast({ title: "Receipt downloading..." })} variant="outline" className="mt-6 rounded-xl">
              Download Receipt
            </Button>
          </motion.div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageHeader title="Complete Your Booking" subtitle="A few quick details and you are ready to go." />
      <div className="container mx-auto max-w-2xl px-4 py-8">
        <div className="mb-8 flex gap-2">
          {bookingSteps.map((s, i) => (
            <div
              key={s}
              className={`flex-1 rounded-full py-2 text-center text-xs font-medium ${i <= step ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
            >
              {s}
            </div>
          ))}
        ...
        <div className="card-surface p-6 space-y-4">
          {step === 0 && (
            <>
              ...
              <Field label="Passport No. (optional)" name="passport" />
            </>
          )}
          {step === 1 && (
            ...
            <div className="space-y-4">
              ...
              {[{ k: "card", l: "Credit/Debit Card", icon: CreditCard }, { k: "upi", l: "UPI", icon: Smartphone }, { k: "netbanking", l: "Net Banking", icon: Building }].map((m) => (
                <label key={m.k} className={`flex items-center gap-3 rounded-xl border p-4 cursor-pointer ${payMethod === m.k ? "border-primary bg-teal-light" : "border-border"}`}>
                  ...
                  <span className="text-sm font-medium text-foreground">{m.l}</span>
                </label>
              ))}
            </div>
          )}
          ...
          <div className="flex gap-3 pt-2">
            ...
            {step < 2 ? (
              <Button onClick={() => { if (validate()) setStep((s) => s + 1); }} className="flex-1 rounded-xl">Next</Button>
            ) : (
              <Button onClick={handlePay} disabled={loading} className="flex-1 rounded-xl">
