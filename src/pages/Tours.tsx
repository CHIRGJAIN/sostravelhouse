    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="gradient-primary py-12 md:py-16">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="mt-2 text-primary-foreground/80">{filteredTours.length} tours found</p>
        </div>
      </div>
                  ...
                  {items.slice(0, 3).map(item => (
                    <div key={item.tour.id} className="flex items-center gap-2 rounded-lg bg-muted p-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[10px] text-muted-foreground">₹{item.tour.pricePerPerson.toLocaleString()}</p>
                      </div>
                    </div>
                  ))}
                </div>
                <p className="mt-3 text-xs font-semibold text-foreground">
                  Est. Total: ₹{items.reduce((s, i) => s + i.tour.pricePerPerson * i.adults, 0).toLocaleString()}
                </p>
              </div>
            </aside>
          )}
