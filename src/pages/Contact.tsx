...
export default function Contact() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validate()) { setSubmitted(true); toast({ title: "Message sent!", description: "We'll get back to you soon." }); }
  };
  ...
  const Field = ({ label, name, type = "text", textarea = false }: { label: string; name: string; type?: string; textarea?: boolean }) => (
    <div>
      ...
      {textarea ? (
        <textarea value={(form as any)[name]} onChange={e => setForm({ ...form, [name]: e.target.value })} rows={4}
        ...
        <input type={type} value={(form as any)[name]} onChange={e => setForm({ ...form, [name]: e.target.value })}
          className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
      )}
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="gradient-primary py-16"><div className="container mx-auto px-4 text-center"><h1 className="font-heading text-3xl font-bold text-primary-foreground">Contact Us</h1></div></div>
      <div className="container mx-auto px-4 py-12">
        <div className="grid gap-8 lg:grid-cols-2">
            ...
            {[
              ...
              <div key={item.label} className="flex items-start gap-3 rounded-xl border border-border bg-card p-4">
                ...
                <div>
                  ...
                  {item.href ? <a href={item.href} className="text-sm text-primary hover:underline">{item.value}</a> : <p className="text-sm text-muted-foreground">{item.value}</p>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-6">
            {submitted ? (
              <div className="py-12 text-center"><p className="text-4xl">✉️</p><h3 className="mt-4 font-heading text-xl font-bold text-foreground">Message Sent!</h3><p className="mt-2 text-muted-foreground">We'll respond within 24 hours.</p></div>
            ) : (
