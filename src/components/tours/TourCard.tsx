...
export default function TourCard({ tour }: Props) {
  const { addItem, removeItem, isInShortlist, addToCompare, isInCompare } = useShortlist();
  const shortlisted = isInShortlist(tour.id);
  const compared = isInCompare(tour.id);

  return (
    <div className="group overflow-hidden rounded-2xl border border-border bg-card transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
